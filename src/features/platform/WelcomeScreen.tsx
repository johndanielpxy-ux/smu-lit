...
export function WelcomeScreen({ onEnter, onBrowse }: WelcomeScreenProps) {
  return <main id="main-content" className="entry entry--welcome">
    ...
    <section className="entry__hero">
      <p className="entry__eyebrow">Firm knowledge, made practical</p>
      <h1>Turn firm knowledge into safer practice.</h1>
      <p>Build interactive training from the legal AI workflows your team already trusts.</p>
      <div className="entry__actions"><button type="button" className="entry__primary" onClick={onEnter}>Enter LAWFLO</button>{onBrowse ? <button type="button" className="entry__link" onClick={onBrowse}>Explore learner training</button> : null}</div>
    </section>
