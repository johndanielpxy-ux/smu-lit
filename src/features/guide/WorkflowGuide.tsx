...
export function WorkflowGuide({ bundle, review, onEvent, onStartSoloReplay }: { bundle: CompiledLawfloBundle; review: LearningReviewResult; onEvent: MatterShiftEventReporter; onStartSoloReplay: () => void }) {
  ...
  return <main id="main-content" className="workflow-guide"><header><span>LAWFLO · Personal workflow guide</span><h1>{bundle.workflowGuide.title}</h1><p>{bundle.workflowGuide.workTrigger}</p><div><strong>Approved by {bundle.manifest.approvedBy}</strong><small>Source set {bundle.manifest.sourceVersion}</small></div></header>
