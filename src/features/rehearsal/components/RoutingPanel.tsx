...
export function RoutingPanel({ rules, state, onOpenRule, onSelect, onRationale, onSubmit }: { rules: PlaybookRule[]; state: RehearsalState; onOpenRule: (id: string) => void; onSelect: (route: Route) => void; onRationale: (rationale: string) => void; onSubmit: () => void }) {
  ...
  return <section className="matter__routing"><div><span className="matter__eyebrow">Current playbook</span><h2>Rules & route</h2></div>
    ...
    {canChooseRoute ? <><div className="matter__routes">{(["business_approval", "legal_review"] as Route[]).map((route) => <button key={route} type="button" className={state.selectedRoute === route ? "is-selected" : ""} aria-label={`Choose ${labels[route]}`} onClick={() => onSelect(route)}>{labels[route]}</button>)}</div><label className="matter__rationale">Explain your route<textarea value={state.routeRationale ?? ""} onChange={(event) => onRationale(event.target.value)} placeholder="State the verified fact and controlling playbook rule" /></label><button className="matter__primary" type="button" disabled={!state.selectedRoute || (state.routeRationale ?? "").trim().length < 24} onClick={onSubmit}>Submit route</button></> : <p className="matter__route-prompt">Open the controlling rule before choosing a route.</p>}
  </section>;
}
