...
interface Envelope { schemaVersion: 2; scope: JourneyScope; state: RehearsalState; }
const tasks = ["intake", "run_ai_review", "verify_findings", "compare_clauses", "apply_playbook", "choose_route", "inspect_audit", "complete"];
const storageKey = (bundleId: string) => `lawflo.rehearsal.${bundleId}.v2`;
const legacyStorageKey = (bundleId: string) => `lawflo.rehearsal.${bundleId}.v1`;
...
export function saveRehearsal(scope: JourneyScope, state: RehearsalState, storage: RehearsalStorage | undefined = browserStorage()): void { if (!storage) return; try { storage.setItem(storageKey(scope.bundleId), JSON.stringify({ schemaVersion: 2, scope, state } satisfies Envelope)); } catch { /* Optional resume. */ } }
export function loadRehearsal(scope: JourneyScope, storage: RehearsalStorage | undefined = browserStorage()): RehearsalState | null { if (!storage) return null; try { const raw = storage.getItem(storageKey(scope.bundleId)); if (!raw) return null; const parsed = JSON.parse(raw) as Partial<Envelope>; if (parsed.schemaVersion !== 2 || !parsed.scope || !sameScope(parsed.scope, scope) || !isState(parsed.state)) return null; return structuredClone(parsed.state); } catch { return null; } }
export function clearRehearsal(bundleId: string, storage: RehearsalStorage | undefined = browserStorage()): void { if (!storage) return; try { storage.removeItem(storageKey(bundleId)); storage.removeItem(legacyStorageKey(bundleId)); } catch { /* Optional reset. */ } }
